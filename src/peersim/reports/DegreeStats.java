...
public class DegreeStats extends GraphObserver {
	...
	private int nextDegree() {
		...
		return switch (type) {
			case "live" -> g.degree(nodeid);
			case "all" -> ((OverlayGraph) g).fullDegree(nodeid);
			case "dead" -> ((OverlayGraph) g).fullDegree(nodeid) - g.degree(nodeid);
			default -> throw new RuntimeException(name + ": invalid type");
		};
	}
