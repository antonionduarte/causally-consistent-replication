...
/**
 * Class that provides functionality for observing graphs. It can efficiently create an undirected version of the graph,
 * making sure it is updated only when the simulation has advanced already, and provides some common parameters.
 */
public abstract class GraphObserver implements Control {
	...
	/**
	 * If defined, the undirected version of the graph will be stored using much more memory but observers will be in
	 * general a few times faster. As a consequence, it will not work with large graphs. Not defined by default. It is a
	 * static property, that is, it affects all graph observers that are used in a simulation. That is, it is not a
	 * parameter of any observer, the name should be specified as a standalone property.
	 *
	 * @config
	 */
...
// ---------------------------------------------------------------------
	...
	/**
	 * If any instance of some extending class defines undir we need to maintain an undir graph. Note that the graph is
	 * stored in a static field so it is common to all instances.
	 */
	...
	/**
	 * Sets {@link #g}. It MUST be called by any implementation of {@link #execute()} before doing anything else.
	 * Attempts to initialize {@link #g} from a pre-calculated graph stored in a static field, but first it checks
	 * whether it needs to be updated. If the simulation time has progressed or it was calculated for a different
	 * protocol, then updates this static graph as well. The purpose of this mechanism is to save the time of
	 * constructing the graph if many observers are run on the same graph. Time savings can be very significant if the
	 * undirected version of the same graph is observed by many observers.
	 */
