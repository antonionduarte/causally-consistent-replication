 ...
 */
public class KebabHeap implements PriorityQueue {
	 ...
	 */
	public KebabHeap(String prefix) {
	...
	public void add(long time, Object event, Node node, byte pid, long priority) {
		...
		while (!stop) {
			...
			long eventTime = time;
			int pre;
			boolean small = times[size / 2] >= eventTime; // true means eventTime is small
			pre = small ? 0 : size - 1;
			while ((small ? pre < size : pre >= 0) && (small ? times[pre] <= eventTime : times[pre] >= eventTime)) {
				if (times[pre] >> pbits == eventTime && nodes[pre] != null && nodes[pre].getID() == node.getID()) {
					if (((eventTime << pbits) | maxPriority()) == times[pre]) {//max priority
						...
						long prePriority = times[pre] & maxPriority();
						if (tempPriority <= prePriority)
							tempPriority = times[pre] & maxPriority() + 1;
					}
				}
				if (small)
					pre++;
